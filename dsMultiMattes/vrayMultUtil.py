...
def setVrayMult(states=None):
    if states:
        i = 0
        loopTime = 1
        multSetupReturn = {"i":0,"loopTime":1,"matteV":""}

        if states["ids"]:
            for number in sorted(states["ids"]):
                multSetupReturn = multSetup(number, multSetupReturn["i"], multSetupReturn["loopTime"], multSetupReturn["matteV"], states)
        if not states["ids"]:
            for number in range(states["startIndex"], states["endIndex"]+1, 1):
                multSetupReturn = multSetup(number, multSetupReturn["i"], multSetupReturn["loopTime"], multSetupReturn["matteV"], states)

def multSetup(number, i, loopTime, matteV="", states=None):
    if i == 0:
    ...
    if i == 3:
        i = 0
        loopTime = loopTime + 1
    return {"i":i,"loopTime":loopTime,"matteV":matteV}
